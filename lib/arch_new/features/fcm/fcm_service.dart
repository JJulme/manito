...
import 'package:manito/arch_new/features/friends/friends_provider.dart';
...
class FCMService {
  ...
  void _handleForegroundMessage(RemoteMessage message) async {
    ...
    switch (messageType) {
      ...
      case 'mission_propose':
        await _handleMissionPropose();
        break;
      case 'update_mission_progress':
        await _handleMissionProgress();
        break;
      case 'update_mission_guess':
        await _handleMissionGuess();
        break;
      case 'update_mission_complete':
        await _handleMissionComplete();
        break;
      case 'insert_comment':
  ...
  Future<void> _handleFriendRequest() async {
    // 친구 신청 뱃지 +1
    _ref.read(badgeProvider.notifier).incrementBadgeLocally('friend_request');

    await _showLocalNotification('friend_request');
  }

  /// 미션 제의
  Future<void> _handleMissionPropose() async {
    // 데이터 새로고침
    await _ref.read(manitoListProvider.notifier).fetchProposeList();
    // 마니또 제의 +1
    _ref.read(badgeProvider.notifier).incrementBadgeLocally('mission_propose');
    await _showLocalNotification('mission_propose');
  }

  /// 마니또가 미션 수락
  Future<void> _handleMissionProgress() async {
    // 데이터 새로고침
    await _ref.read(missionListProvider.notifier).fetchMyMissions();
    // 뱃지 증가
    _ref.read(badgeProvider.notifier).incrementBadgeLocally('mission_accept');

    await _showLocalNotification('mission_accept');
  }

  /// 마니또가 미션 완료
  Future<void> _handleMissionGuess() async {
    // 데이터 새로고침
    await _ref.read(missionListProvider.notifier).fetchMyMissions();
    // 뱃지 증가
    _ref.read(badgeProvider.notifier).incrementBadgeLocally('mission_guess');

    await _showLocalNotification('mission_guess');
  }

  /// 생성자가 추측 완료
  Future<void> _handleMissionComplete() async {
    // 데이터 새로고침
    await _ref.read(manitoListProvider.notifier).fetchGuessList();
    await _ref.read(postsProvider.notifier).fetchPosts();
    // 뱃지 증가
    _ref.read(badgeProvider.notifier).incrementBadgeLocally('mission_complete');
  ...
  Future<void> _handleNewComment(RemoteMessage message) async {
    ...
    _ref.read(badgeProvider.notifier).addBadgeComment(missionId);
    // 댓글 알림 표시
    // 댓글 알림 표시
