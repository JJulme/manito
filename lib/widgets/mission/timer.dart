...
class TimerWidget extends StatelessWidget {
  final String targetDateTimeString; // 타이머 종료 시간
  ...
  const TimerWidget({
    super.key,
    required this.targetDateTimeString,
  ...
  Widget build(BuildContext context) {
    ...
    final TimerController controller =
        TimerController(targetDateTimeString, onTimerComplete: onTimerComplete);

    return Obx(
      () {
        // 남은 시간 포맷팅
        var textList =
            controller.formatRemainingTime(controller.remainingTime.value);
        return Row(
...
class TimerController extends GetxController {
  ...
  TimerController(this.targetDateTimeString, {this.onTimerComplete}) {
    startCountdown(); // 타이머 시작
  }

  Rx<Duration> remainingTime = Duration.zero.obs; // 남은 시간
  late DateTime _targetDateTime; // 타이머 종료 시간
  Timer? _timer; // 타이머 객체
  ...
  void startCountdown() {
    ...
    _targetDateTime = formatter.parse(targetDateTimeString);
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) async {
      ...
      if (now.isAfter(_targetDateTime)) {
  ...
  List<String> formatRemainingTime(Duration duration) {
    ...
    String minutes =
        (duration.inMinutes % 60).toString().padLeft(2, '0'); // 남은 분
    String seconds =
        (duration.inSeconds % 60).toString().padLeft(2, '0'); // 남은 초
